...
class scMultiCluster(nn.Module):
    def __init__(self, input_dim1, input_dim2, device="cuda"):
    ...
    def pretrain_autoencoder(self, X1, X2, X1_raw, X2_raw, epochs=400, file=None, ad_out=16):
        ...
        for epoch in range(epochs):
            for batch_data in dataloader:
                ...
                loss = 0.5*recon_loss + zinb_loss + 0.1*discriminator_loss
        ...
        if y is not None:
            ...
            for epoch in range(num_epochs):
                for _, batch_data in enumerate(dataloader):
                    ...
                    # overall loss
                    loss = 0.5*recon_loss + zinb_loss + cluste_loss
            ...
            for epoch in range(num_epochs):
                for _, batch_data in enumerate(dataloader):
                    ...
                    # overall loss
                    loss = 0.2*recon_loss + 0.8*zinb_loss + cluste_loss
