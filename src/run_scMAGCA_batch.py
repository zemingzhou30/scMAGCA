...
from utils import *
from scMAGCA_batch import scMultiCluster
...
if __name__ == "__main__":
    ...
    parser.add_argument('--f1', default=2000, type=float, help='Number of ADT/ATAC after feature selection')
    parser.add_argument('--f2', default=2000, type=float, help='Number of mRNA after feature selection')
    ...
    x1 = np.array(sc.read_h5ad('../datasets/GSE164378_adt.h5ad').to_df()).astype('float32')
    x2 = np.array(sc.read_h5ad('../datasets/GSE164378_rna.h5ad').to_df()).astype('float32')
    y = np.array(pd.read_csv('../datasets/GSE164378_label.csv')['Cluster']).astype('float32')
    b = np.array(pd.read_csv('../datasets/GSE164378_batch.csv')['Batch']).astype('float32')
    enc = OneHotEncoder()
    ...
    model = scMultiCluster(input_dim1=input_size1, input_dim2=input_size2, n_batch=2, device=args.device).to(args.device)
    print(str(model))
